local path_sep = require("capucin").path_sep
...
function M.compile(flavour)
	local theme = require("capucin.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
local h = vim.api.nvim_set_hl]],
	...
	if vim.g.capucin_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/capucin_error.lua"
		print(string.format(
			[[Capucin (error): Most likely some mistake made in your capucin config
