	...
	for integration in pairs(integrations) do
		...
		if cot then
			final_integrations = vim.tbl_deep_extend(
				"force",
				final_integrations,
				require("capucin.groups.integrations." .. integration).get()
			)
		end
	end
...
function M.apply(flavour)
	flavour = flavour or require("capucin").flavour
	-- Borrowing global var
	_G._O = O
	_G._C = C
	_G._U = U

	_G.O = require("capucin").options
	_G.C = require("capucin.palettes").get_palette(flavour)
	_G.U = require "capucin.utils.colors"
	...
	local theme = {}
	theme.syntax = require("capucin.groups.syntax").get()
	theme.editor = require("capucin.groups.editor").get()
	theme.integrations = get_integrations() -- plugins
	theme.terminal = require("capucin.groups.terminal").get() -- terminal colors
	local user_highlights = require("capucin").options.highlight_overrides
